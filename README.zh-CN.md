...
- 单命令详情入口：`GET /api/v1/commands/:command`
- 命令清单元数据入口：`GET /api/v1/manifest`
- 事件流入口：`GET /api/v1/events/stream`（SSE）
- 健康检查：`GET /health`
- 可读首页：`GET /`
- 首页内置交互控制台（可选命令、改参数、直接执行）
- 可选按命令策略控制（`POLYGATE_DISABLED_COMMANDS`、`POLYGATE_FORCE_AUTH_COMMANDS`）
- 私有命令应用层 API Token 鉴权（`authRequired: true`）
- 可选 Webhook 推送命令执行事件
...
- `独立 docs 命令手册`：Implemented
- `按命令粒度鉴权策略模板`：Implemented
- `Webhook + SSE 命令事件推送`：Implemented
- `私有命令应用层 API Token 鉴权`：Implemented
...
一次性返回所有命令元数据，适合自动化接入与 SDK 生成。

### 事件流（SSE）

- `GET /api/v1/events/stream`

以 `text/event-stream` 形式实时返回 `command.execution` 事件。

### 首页

- `GET /`

返回轻量 HTML 首页，包含核心说明、常用接口入口和 curl 示例。
支持语言切换：`/?lang=en` 与 `/?lang=zh-CN`。
...
### 链上写命令集成测试（approve/ctf）

这组测试会发真实链上交易，默认关闭。

必填环境变量：

- `POLYMARKET_PRIVATE_KEY`（有 gas 的钱包）
- `POLYGATE_CHAIN_TEST_SPLIT_CONDITION_ID`（用于 split/merge 的已准备 condition）
- `POLYGATE_CHAIN_TEST_REDEEM_CONDITION_ID`（用于 redeem 的已结算 condition）

可选环境变量：

- `POLYGATE_CHAIN_TEST_API_TOKEN`（默认：`chain-test-token`）
- `POLYGATE_CHAIN_TEST_SPLIT_AMOUNT`（默认：`0.1`）
- `POLYGATE_CHAIN_TEST_PARTITION`（默认：`1,2`）
- `POLYGATE_CHAIN_TEST_REDEEM_INDEX_SETS`（默认：`1,2`）
- `POLYGATE_CHAIN_TEST_COLLATERAL`
- `POLYGATE_CHAIN_TEST_PARENT_COLLECTION_ID`（默认：零 bytes32）
...
- `POLYGATE_DISABLED_COMMANDS`（逗号分隔的命令禁用列表）
- `POLYGATE_FORCE_AUTH_COMMANDS`（逗号分隔的强制钱包鉴权命令列表）
- `POLYGATE_REQUIRE_API_TOKEN_FOR_PRIVATE`（默认 `true`）
- `POLYGATE_API_TOKENS`（私有命令可用 token 列表，逗号分隔）
- `POLYGATE_API_TOKEN_HEADER`（默认 `x-polygate-token`）
- `POLYGATE_WEBHOOK_URL`（可选命令事件接收地址）
- `POLYGATE_WEBHOOK_BEARER_TOKEN`（可选 webhook bearer token）
- `POLYGATE_WEBHOOK_TIMEOUT_MS`（webhook 超时，默认 `5000`）
- `POLYGATE_SSE_HEARTBEAT_MS`（SSE 心跳间隔，默认 `15000`）
...
- `x-polygate-token`（或你配置的 `POLYGATE_API_TOKEN_HEADER`）
...
- 生产保持 `POLYGATE_REQUIRE_API_TOKEN_FOR_PRIVATE=true` 并配置强 token
- 不要把交易路由直接裸露在公网
- 建议读接口与交易接口分路由或分域名
...
- `Implemented`：`approve.set` / `ctf.split` / `ctf.merge` / `ctf.redeem` 链上集成测试（环境变量门控）
- `Implemented`：按类别拆分 Curl 命令手册到 `docs/curl`
- `Implemented`：按命令粒度的鉴权策略模板
- `Planned`：分页辅助和更强类型化 SDK facade
- `Implemented`：Webhook 与 SSE 命令事件扩展
